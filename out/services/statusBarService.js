...
const fileVersionService_1 = require("./fileVersionService");
/**
 * 状态栏服务类
 */
class StatusBarService {
     ...
     */
    static async updateStatusBar(editor) {
        ...
        try {
            ...
            const projectRoot = configReader_1.ConfigReader.findProjectRootForStatusBar(filePath);
            if (!projectRoot) {
                // 找不到项目根目录，显示默认文本
                // 调试信息：可以在开发者工具中查看
            ...
            let config = null;
            try {
                config = configReader_1.ConfigReader.readConfig(projectRoot);
                if (config) {
            ...
            if (config && config.files && config.files.length > 0) {
                ...
                for (const fileConfig of config.files) {
                    try {
                        ...
                        const currentVersion = fileVersionService_1.FileVersionService.getCurrentVersion(configFilePath, fileConfig);
                        if (currentVersion) {
...
StatusBarService.currentProjectRoot = null;
