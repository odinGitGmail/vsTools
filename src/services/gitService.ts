...
export class GitService {
     ...
     * @returns Git 提交信息列表
     */
    public static async getCommitInfo(projectRoot: string): Promise<string[]> {
        // 检查是否是 Git 仓库
     ...
     * @returns 格式化的提交信息字符串
     */
    public static async getDetailedCommitInfo(projectRoot: string): Promise<string> {
        // 检查是否是 Git 仓库
        const gitDir = path.join(projectRoot, '.git');
        if (!fs.existsSync(gitDir)) {
            throw new Error('当前目录不是 Git 仓库');
        ...
        try {
            ...
            const { stdout, stderr } = await execPromise(
                'git log --all --pretty=format:"%n========================================%n提交哈希: %H%n版本号（Tag）: %d%n作者: %an <%ae>%n日期: %ad%n提交说明（完整，包含换行）:%n%B" --date=iso',
                {
                    cwd: projectRoot,
                    maxBuffer: 10 * 1024 * 1024 // 10MB buffer
                }
     ...
     */
    public static async getCommitsForChangelog(projectRoot: string): Promise<Array<{
        version: string;
        date: string;
        message: string;
        author: string;
        ...
        try {
            ...
            const { stdout, stderr } = await execPromise(
                'git log --all --pretty=format:"CHANGELOG_SEPARATOR%n%d%n%H%n%ad%n%an%n%B" --date=iso',
                {
                    cwd: projectRoot,
                    maxBuffer: 10 * 1024 * 1024 // 10MB buffer
                }
     ...
     */
    public static async generateChangelog(projectRoot: string): Promise<string> {
     ...
     */
    public static findGitRoot(filePath: string): string | null {
